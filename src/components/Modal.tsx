...
const Modal: React.FC<ModalProps> = ({ showModal, onClose, orders, updateOrders }) => {
  ...
  const cleanFields = () => {
    ...
    console.log('va ELe', valueElement, quantityElement, productElement);
  
    if (valueElement) {
      // O elemento com ID "value-input" foi encontrado
      valueElement.value = 'ÇLAKJDSFA'; // Definindo o valor para uma string vazia
      setValue('');
    }
    if (quantityElement) {
      // O elemento com ID "value-input" foi encontrado
        ...
        {isAddButtonVisible && (
          <div className='close-order-section' style={{ marginTop: '24px' }}>
            ...
            <section className='svg-label-btn' onClick={handleAddProduct}>
              ...
              <label style={{ fontSize: '10px' }}>CONCLUIR</label>
            </section>
